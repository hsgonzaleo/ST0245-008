...
class arbol():  
    ...
    def get_nivel(self, a, nivel = 1):  # Función que consigue el nivel del árbol
        if a == None:
            return 0
        if a.left != None:
            nivel = self.get_nivel(a.left, nivel + 1)
        if a.right != None:
            d = self.get_nivel(a.right, nivel + 1)
            if d > nivel:
                nivel = d
        return nivel
...
while True:
    ...
    if opc == '1':
        ...
        print("\nEl nivel del árbol es %d...\n" % tree.get_nivel(tree.root))   
    elif opc == '8':
        print("\nElegiste salir...\n")
        os.system("pause")
        break
